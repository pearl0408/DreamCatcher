...
public class FeedDrag : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (isTriggering)   //횟대와 충돌 중이라면
        {
            ...
            //새 종류 랜덤 설정
            int randomBird = GameObject.FindGameObjectWithTag("FeedManager").GetComponent<BirdImage>().SelectBirdType(feedNum); //확률에 따라 랜덤으로 해당 먹이의 새를 정함
            ...
            GameObject.FindGameObjectWithTag("FeedManager").GetComponent<BirdImage>().SelectBirdImage();    //새 오브젝트 이미지 변경(비활성화 상태)
